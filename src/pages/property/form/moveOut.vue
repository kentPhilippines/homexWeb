...
<template>
  <div>
      ...
      <div class="search-head">
        <a-row>
          <a-col :span="4" class="search-head-logo">
            <img src="../../../../static/img/move_out.svg">
            <span>MOVE OUT</span>
          </a-col>
          <a-col :span="16">
          ...
          <a-col :span="4">
            <a-button v-if="userRole == 6" class="search-addForm" type="primary" @click="showChooseModal()">New Move Out Form</a-button>
          </a-col>
        </a-row>
      </div>
            ...
            <table class="form-table">
              <tbody class="ant-table-tbody">
                <tr>
                  ...
                  <td style="width:25%;text-align:center;">{{ forms.reason }}</td>
    ...
    <div v-else-if="type == 'form'">
      <div class="form-body">
        <div class="form-body-title">Move Out Form</div>
        <a-form class="form-body-form" 
          ...
          <a-form-item label="Tenant" required>
            <div class="form-body-form-input" v-for="(tenant, index) in hasTenantList" :key="index">
              <a-input class="form-body-form-input-small" v-model="tenant.name" :disabled="true"></a-input>
              <a-checkbox @change="changeChooseTenant($event, tenant)"></a-checkbox>
            </div>
          </a-form-item>
          ...
          <a-form-item label="Reason" required>
            <a-input class="form-body-form-input" v-model="addTenantInfo.reason"></a-input>
          </a-form-item>
          
          <div v-if="hasTenant" class="redText ant-form-explain ant-form-extra" style="width:600px;left:0px;top:inherit;">Please select at least one tenant</div>
    ...
    <a-modal v-model="isReject" title="Reject" :footer="null" @cancel="cancelReject">
      <a-form @submit.self="rejectForm" :form="moveoutform" layout="inline" ref="form" class="search-form-set">
...
export default {
  ...
  data () {
    return {
      ...
      isReject: false,
      moveoutform: this.$form.createForm(this),
      rejectInfo: {},
  ...
  methods: {
    ...
    getForms() {
      ...
      this.$http({
        url: this.$http.adornUrl('/dev-api/getMoveOutList'),
        method: 'get',
        params: this.$http.adornParams({...params})
      }).then(res => {
        if (res.status === 200) {
    ...
    approveForm(forms) {
      ...
      this.$http({
        url: this.$http.adornUrl('/dev-api/changeMoveOutStatus'),
        method: 'post',
    ...
    cancelReject() {
      this.isReject = false
      this.rejectInfo = {}
      this.moveoutform.resetFields()
    },
    rejectForm(e) {
      e.preventDefault()
      this.moveoutform.validateFields(async (err, values) => {
        if (!err) {
          let params = {
            userID: this.userID,
            oldstatus: 'pending',
            ...
            reason: this.moveoutform.getFieldValue('reason')
          }
          this.$http({
            url: this.$http.adornUrl('/dev-api/changeMoveOutStatus'),
            method: 'post',
    ...
    deleteForm(forms) {
      ...
      this.$http({
        url: this.$http.adornUrl('/dev-api/deleteMoveOut'),
        method: 'post',
        data: this.$http.adornData({...params})
      }).then(res => {
        if (res.status === 200) {
    ...
    getTenantList() {
      ...
      this.$http({
        url: this.$http.adornUrl('/dev-api/getUnitTenantList'),
        method: 'get',
        params: this.$http.adornParams({...params})
      }).then((res) => {
        if (res.status == 200) {
          this.hasTenantList = []
          this.hasTenantList = res.data.data.tenantList
        } else {
          this.$message.error('failed get tenantList')
        }
      })
      // this.hasTenantList = []
      // this.hasTenantList = [
      //   {id: '1', name: 'Beacon Condo'},
      //   {id: '2', name: 'Avida Towers San Lorenzo'},
      //   {id: '3', name: 'Avida Towers San Lorenzo'},
      //   {id: '4', name: 'Beacon Condo'}
      // ]
    },
    showChooseModal() {
      this.getUnitList()
    ...
    changeChooseTenant(e, tenant) {
      let index = this.chooseTenantList.indexOf(tenant.name)
      if (e.target.checked) {
        if (index == -1) {
          this.chooseTenantList.push(tenant.name)
        }
      } else {
        if (index > -1) {
          this.chooseTenantList.splice(index, 1)
        }
      }
      if (this.chooseTenantList.length > 0) {
    ...
    sendForm() {
      let len = this.chooseTenantList.length
      if (len <= 0) {
        this.hasTenant = true
      } else {
        this.hasTenant = false
        if (!this.addTenantInfo.phone || !this.addTenantInfo.moveout || !this.addTenantInfo.reason) {
          this.isLack = true
        } else {
          this.isLack = false
        }
      ...
      if (!this.hasTenant && !this.isLack) {
        let params = {
          ...
          tower: this.chooseTower,
          unit: this.chooseUnit,
          tenantList: this.chooseTenantList,
          tenantInfo: this.addTenantInfo
        }
        this.$http({
          url: this.$http.adornUrl('/dev-api/addMoveOut'),
          method: 'post',
          data: this.$http.adornData({...params})
        }).then(res => {
          if (res.status === 200) {
