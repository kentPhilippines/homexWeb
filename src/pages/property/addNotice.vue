    ...
    <div class="add-notice">
      ...
      <a-row class="add-search">
        ...
        <a-col :span="12">
          ...
          <div class="add-search-content" style="display:flex;align-items:center;margin-top:19px;">
            <a-select v-model="towerChoose" style="width:40%;" @change="getUnitList()">
      ...
      <a-row class="add-search">
        <a-col :span="24" :style="{ textAlign: 'right' }">
          <a-button type="primary" @click="addNotice">Send</a-button>
        </a-col>
      </a-row>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    addNotice() {
      ...
      this.$http({
        url: this.$http.adornUrl('/api/notice/addNotice'),
        method: 'post',
        data: this.$http.adornData({...params})
      }).then((res) => {
        if (res.status == 200) {
          this.$message.success('success add notice')
          this.$router.push('/property/dashboard')
        } else {
          this.$message.error('failed add notice')
        }
      })
    }
  },
